...
(function init() {
    ...
    function setcookie() {
        if (key === runkey) {
            document.cookie = 'meringue=' + encodeURIComponent(JSON.stringify(setting)) + '; max-age=86400000';
        }
    }
        ...
        if (key === runkey) {
            ...
            switch (m.type) {
                case 'top': {
                    ...
                    rankingbox.innerHTML = '<hr><p><span id="viewers">' + m.data.date + '時点で記録表を見ている人数: ' + m.data.viewers + '</span></p><hr><p>本日はお越しくださりありがとうございます。</p><p>ページ上部の「得点表を選択」の隣にある四角を押して、記録が見たい種目を選択してください！</p>';
                    break;
                }
                case "getboard": {
                ...
                case 'newscore': {
                    ...
                    board[m.data.boardname].score.forEach((s, i) => {
                        ...
                        insertscore(rankdata.rank, s.name, s.score, board[m.data.boardname].counter, s.id);
                    });
                    break;
                }
                case 'warndelete':
                ...
                case 'deleteboard':
                    if (category.value === m.data.boardname) {
                        notice('「' + m.data.boardname + '」の記録は削除されました。（' + m.data.date + '）');
                        send({
                            type: "top"
                        });
                    } else {
                ...
                case 'viewers': {
                    ...
                    if (element != null) {
                        element.innerText = m.data.date + '時点で記録表を見ている人数: ' + m.data.viewers;
                    }
                    break;
                }
            }
